...
{
    printf("DEBUG: connmgr_flushed(p->connmgr) %p\n",mgr);
	if (mgr->fail_open) {
        fail_open_flushed(mgr->fail_open);
    }
    //printf("DEBUG: Inside connmgr_flushed....\n");
    ...
    if (!connmgr_has_controllers(mgr)
        ...
        struct match match;
        //printf("DEBUG: Inside connmgr_flushed...stage1.\n");
        ofpbuf_init(&ofpacts, OFPACT_OUTPUT_SIZE);
        ofpact_put_OUTPUT(&ofpacts)->port = OFPP_NORMAL;
        ofpact_pad(&ofpacts);
        //printf("DEBUG: Inside connmgr_flushed...stage2.\n");
        match_init_catchall(&match);
        printf("DEBUG: Inside connmgr_flushed...stage3 %p.\n",mgr->ofproto);
        ofproto_add_flow(mgr->ofproto, &match, 0, ofpacts.data, ofpacts.size);
        printf("DEBUG: Inside connmgr_flushed...stage4.\n");
        ofpbuf_uninit(&ofpacts);
    }
    //printf("DEBUG: Inside connmgr_flushed...stage5.\n");
}

...
#else
                fu.priority=SGX_cls_rule_priority(&rule->cr);
#endif
