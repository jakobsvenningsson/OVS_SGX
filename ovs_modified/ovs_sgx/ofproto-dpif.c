...
//1. First we need to determine the size of the buffer
  int buf_size;
  buf_size=SGX_desfet_ccfes_c();
  if(buf_size){
	  int lp;
	  struct cls_rule *buf[buf_size];
	  SGX_desfet_ccfes_r(buf,buf_size);
...
#else
    if (SGX_cls_rule_priority(&rule->up.cr) == FAIL_OPEN_PRIORITY) {
#endif
    	/*
...
facet_check_consistency(struct facet *facet)
{
    ...
    if (rule != facet->rule) {
        if (!VLOG_DROP_WARN(&rl)) {
            ...
            unsigned int priority_sgx= SGX_cls_rule_format(&facet->rule->up.cr,&megamatch);
            match_format(&megamatch,&s,priority_sgx);
#endif
...
#else
            struct match megamatch1;
            unsigned int priority_sgx1= SGX_cls_rule_format(&rule->up.cr,&megamatch1);
            match_format(&megamatch,&s,priority_sgx1);
#endif
            ds_put_cstr(&s, ")\n");
...
#else
    if (SGX_is_sgx_other_table(table_id)) {
    	tag_type rule_tag;
    	if(rule && rule->tag){
    		rule_tag=rule->tag;
    	}else{
    		uint32_t hash;
    		hash=SGX_rule_calculate_tag_s(table_id,&ctx->xin->flow,table->basis);
    		if(hash){
...
#else
    int todo;
    todo=SGX_table_update_taggable(rule->up.table_id);
                  ...
                  const struct rule_dpif *rule)
{
    ...
    struct match megamatch;
    unsigned int priority_sgx= SGX_cls_rule_format(&rule->up.cr,&megamatch);
