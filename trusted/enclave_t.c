...
static sgx_status_t SGX_CDECL sgx_ecall_start_poller(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_start_poller_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_start_poller_t* ms = SGX_CAST(ms_ecall_start_poller_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	async_ecall* _tmp_data = ms->ms_data;



	ms->ms_retval = ecall_start_poller(_tmp_data);
...
static sgx_status_t SGX_CDECL sgx_ecall_cls_rule_init(void* pms)
{
	...
	sgx_status_t status = SGX_SUCCESS;
	struct cls_rule* _tmp_o_cls_rule = ms->ms_o_cls_rule;
	const struct match* _tmp_match = ms->ms_match;
...
static sgx_status_t SGX_CDECL sgx_ecall_cls_rule_hash(void* pms)
{
	...
	sgx_status_t status = SGX_SUCCESS;
	const struct cls_rule* _tmp_o_cls_rule = ms->ms_o_cls_rule;
...
static sgx_status_t SGX_CDECL sgx_ecall_cls_rule_equal(void* pms)
{
	...
	sgx_status_t status = SGX_SUCCESS;
	const struct cls_rule* _tmp_o_cls_rule_a = ms->ms_o_cls_rule_a;
	const struct cls_rule* _tmp_o_cls_rule_b = ms->ms_o_cls_rule_b;
...
static sgx_status_t SGX_CDECL sgx_ecall_cls_find_match_exactly(void* pms)
{
	...
	sgx_status_t status = SGX_SUCCESS;
	const struct match* _tmp_target = ms->ms_target;
	struct cls_rule** _tmp_o_cls_rule = ms->ms_o_cls_rule;
...
static sgx_status_t SGX_CDECL sgx_ecall_oftable_enable_eviction(void* pms)
{
	...
	sgx_status_t status = SGX_SUCCESS;
	const struct mf_subfield* _tmp_fields = ms->ms_fields;
...
static sgx_status_t SGX_CDECL sgx_ecall_collect_ofmonitor_util_c(void* pms)
{
	...
	sgx_status_t status = SGX_SUCCESS;
	const struct minimatch* _tmp_match = ms->ms_match;
...
static sgx_status_t SGX_CDECL sgx_ecall_cls_rule_is_loose_match(void* pms)
{
	...
	sgx_status_t status = SGX_SUCCESS;
	struct cls_rule* _tmp_o_cls_rule = ms->ms_o_cls_rule;
	const struct minimatch* _tmp_criteria = ms->ms_criteria;
...
static sgx_status_t SGX_CDECL sgx_ecall_cls_lookup(void* pms)
{
	...
	sgx_status_t status = SGX_SUCCESS;
	struct cls_rule** _tmp_o_cls_rule = ms->ms_o_cls_rule;
	const struct flow* _tmp_flow = ms->ms_flow;
...
sgx_status_t SGX_CDECL ocall_myenclave_sample(const char* str)
{
	...
	ocalloc_size -= sizeof(ms_ocall_myenclave_sample_t);
