...
sgx_status_t ecall_table_mflows_set(sgx_enclave_id_t eid, int table_id, unsigned int value)
{
	...
	ms.ms_table_id = table_id;
	ms.ms_value = value;
	status = sgx_ecall(eid, 32, &ocall_table_myenclave, &ms);
	return status;
}
...
{
	sgx_status_t status;
	status = sgx_ecall(eid, 33, &ocall_table_myenclave, NULL);
	return status;
}
...
sgx_status_t ecall_rule_calculate_tag_s(sgx_enclave_id_t eid, uint32_t* retval, int id, const struct flow* flow, uint32_t secret)
{
	...
	ms.ms_flow = flow;
	ms.ms_secret = secret;
	status = sgx_ecall(eid, 53, &ocall_table_myenclave, &ms);
	if (status == SGX_SUCCESS && retval) *retval = ms.ms_retval;
	return status;
}
...
{
	sgx_status_t status;
	status = sgx_ecall(eid, 54, &ocall_table_myenclave, NULL);
	return status;
}
...
sgx_status_t ecall_ofproto_get_vlan_r(sgx_enclave_id_t eid, uint16_t* buf, int elem)
{
	...
	ms.ms_buf = buf;
	ms.ms_elem = elem;
	status = sgx_ecall(eid, 59, &ocall_table_myenclave, &ms);
	return status;
}
